...
class AbsensiController extends Controller
{
    ...
    public function store_qr(Request $request)
    {
        ...
        if ($absensi->count() > 0) {
            return redirect()->route('mahasiswa')->with('error', 'Anda telah melakukan presensi.');
        }
        ...
        return redirect()->route('mahasiswa')->with('success', 'Presensi berhasil disimpan.');
    }
    ...
    public function tutup_absen(Request $request)
    {
        $kelas = kelas::find($request->kelas_id);
